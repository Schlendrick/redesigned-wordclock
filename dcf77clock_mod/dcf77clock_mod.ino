...
void serialDumpTime(void) {
  ...
  //  lcd.setCursor( 0, 1);
  //  lcd.print( String(hh) +":"+ String(mm) +":"+ String(ss) +"_"+ String(day) +"."+ String(mon) +"."+ String(year) );
}
...
void scanSignal(void) {
  if (DCFSignalState == 1) {
    ...
    lcd.setCursor( 0, 0);
    lcd.print( String(previousFlankTime) + " detected Sig");
    lcd.setCursor( 0, 1);
    lcd.print( "T" + String(hh) + ":" + String(mm) + ":" + String(ss) + "_" + String(day) + "." + String(mon) + "." + String(year % 2000) );
#endif
  }
  else {
    /* or a falling flank */
